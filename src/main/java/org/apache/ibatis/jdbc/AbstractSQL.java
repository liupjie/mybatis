 ...
 * 依赖与内部类实现SQL语句的拼接工作
 *
 * AbstractSQL类 中存在大量的全大写字母命名的方法，如 UPDATE、SET等，这是为了照 顾用户的使用习惯。
 * 因为通常我们在书写 SQL语句时会将 UPDATE、SET 等关键字大写。
 * @param <T>
 */
public abstract class AbstractSQL<T> {
    ...
    private static class SafeAppendable {
         ...
         * 向主串拼接一段字符串
         * @param s 被拼接的字符串
         * @return SafeAppendable 内部类自身
         */
        public SafeAppendable append(CharSequence s) {
            try {
                // 需要拼接的字符串长度不为零，则拼完后的主串也不是空了
                if (empty && s.length() > 0) {
                    empty = false;
                }
                // 拼接
    ...
    /**
     *  SQLStatement内部类可以完整地表述出一条 SQL语句，它的主要属性如代码所示。可以看出，这些属性完整地表述了一条 SQL语句所 需要的各种片段信息。
     */
    private static class SQLStatement {
        ...
        /**
         * sql方法，能够根据不同的语 句类型调用相应的子方法将语句片段信息拼接成一个完整的 SQL语句
         *
