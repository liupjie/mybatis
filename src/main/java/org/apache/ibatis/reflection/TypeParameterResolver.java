...
public class TypeParameterResolver {
     ...
     * @return 解析结果 The parameter types of the method as an array of {@link Type}s. If they have type parameters in the declaration,<br>
     * they will be resolved to the actual runtime {@link Type}s.
     */
     ...
     * @param srcType           该变量所属的类型
     * @param declaringClass    定义该变量的类
     * @return 参数化类型的实际结果
     */
    private static ParameterizedType resolveParameterizedType(ParameterizedType parameterizedType, Type srcType, Class<?> declaringClass) {
        // 变量的原始类型，比如 public List<T> getInfo() {} 中为List
        Class<?> rawType = (Class<?>) parameterizedType.getRawType();
        // 获取类型参数，比如 public List<T> getInfo() {} 中只有一个类型参数T
        Type[] typeArgs = parameterizedType.getActualTypeArguments();
        // args数组存储类型参数的实际类型
        Type[] args = new Type[typeArgs.length];
        for (int i = 0; i < typeArgs.length; i++) { // 依次处理每一个类型参数
            if (typeArgs[i] instanceof TypeVariable) {
                // 类型参数是类型变量，例如parameterizedType为List<T>则属于这种情况
                args[i] = resolveTypeVar((TypeVariable<?>) typeArgs[i], srcType, declaringClass);
            } else if (typeArgs[i] instanceof ParameterizedType) {
                // 类型参数是参数化类型，例如parameterizedType为List<List<T>>则属于这种情况
                args[i] = resolveParameterizedType((ParameterizedType) typeArgs[i], srcType, declaringClass);
            } else if (typeArgs[i] instanceof WildcardType) {
                // 类型参数是通配符泛型，例如parameterizedType为List<？extends Number>则属于这种情况
                args[i] = resolveWildcardType((WildcardType) typeArgs[i], srcType, declaringClass);
            } else {
                // 类型参数是确认的类型，例如parameterizedType为List<String>则属于这种情况
                args[i] = typeArgs[i];
            }
        }
    ...
    /**
     * 解析泛型变量的实际结果
     * resolveTypeVar方法会尝试通过继承关系等确定泛型变量的具体结果
     *
     * @param typeVar        泛型变量
    ...
    private static Type resolveTypeVar(TypeVariable<?> typeVar, Type srcType, Class<?> declaringClass) {
        ...
        // 获取变量所属的父类，在该示例中，变量属于Student类，其父类为User<Number>类
